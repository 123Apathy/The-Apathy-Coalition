...
import { createRepoSelfImprovementJob } from '../lib/tools/repo-self-improvement.js';
...
import { writeSkillsFromExecution } from '../lib/skills/skill-writer.js';
import { emitControlTowerEvent } from '../lib/control-tower/event-bus.js';
...
async function handleWebhook(request) {
  ...
  const { job, job_type } = body;
  if (!job && job_type !== 'repo_self_improvement') {
    return Response.json({ error: 'Missing job field' }, { status: 400 });
  }

  try {
    const result = job_type === 'repo_self_improvement'
      ? await createRepoSelfImprovementJob({ focus: job || '' })
      : await createJob(job);
...
async function handleGithubWebhook(request) {
  ...
  try {
    ...
    writeSkillsFromExecution(results).catch((err) => {
      console.warn('[skills] failed to write execution skills:', err.message);
    });
    ...
    // Refresh codebase intelligence after autonomous improvements or major code changes.
    if ((payload.changed_files || []).some((file) => !String(file).startsWith('logs/'))) {
      import('../lib/codebase-intelligence/codebase-memory-writer.js')
        .then(({ runCodebaseIntelligence }) => runCodebaseIntelligence())
        .catch((err) => {
          console.warn('[codebase intelligence] post-job refresh failed:', err.message);
        });
    }
