...
- **Job Runner isolation** — In the standard Docker Compose deployment, the Event Handler talks to an internal Job Runner API instead of mounting `/var/run/docker.sock` directly. Docker control is isolated behind policy checks and runtime limits.
...
- **Privileged Job Runner** — The Job Runner improves isolation for the Event Handler, but it is still a privileged internal service with Docker Engine access. If that service is compromised, the host blast radius can still be high.
