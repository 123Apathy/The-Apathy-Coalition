...
docker-compose up      # Start Traefik + event handler + job runner + runner
```

The standard Compose deployment now includes a `job-runner` service. The Event Handler talks to that internal service over HTTP for container control, and only the Job Runner mounts the Docker socket.
...
| **job-runner** | `stephengpope/thepopebot:event-handler-${THEPOPEBOT_VERSION}` | Internal Docker control plane that launches and supervises agent containers |
...
The Job Runner is the main host-security improvement in the default deployment. It is the only service that mounts `/var/run/docker.sock` for interactive workspaces, headless coding, and cluster workers. The Event Handler keeps its project bind mount, but no longer needs direct Docker Engine access.
