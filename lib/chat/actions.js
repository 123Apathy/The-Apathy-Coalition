...
export async function createWorkspaceRecord(input) {
  ...
  return createWorkspace(user.id, input);
}
...
export async function getWorkspaceDetail(workspaceId) {
  ...
  const {
    ...
    listWorkspaceMemory,
  } = await import('../db/workspaces.js');

  const workspace = getWorkspaceForUser(workspaceId, user.id);
...
export async function createWorkspaceInviteRecord(workspaceId, input) {
  const user = await requireAuth();
  ...
  if (!workspace || workspace.role !== 'owner') {
    throw new Error('Only workspace owners can create invites.');
  }

  const token = randomUUID();
  const invite = createWorkspaceInvite(workspaceId, user.id, {
  ...
  addWorkspaceActivity(workspaceId, user.id, 'workspace.invite.created', {
    email: input.email || null,
    role: input.role || 'collaborator',
  });
  ...
  return {
    ...invite,
    inviteUrl: `${process.env.APP_URL || 'http://localhost:3000'}/workspaces/invite/${token}`,
  };
}

export async function createWorkspaceProjectRecord(workspaceId, input) {
  const user = await requireAuth();
  const {
    getWorkspaceForUser,
  ...
  const project = createWorkspaceProject(workspaceId, user.id, input);
  addWorkspaceActivity(workspaceId, user.id, 'workspace.project.created', { name: input.name });
  return project;
}

export async function createWorkspaceTaskRecord(workspaceId, input) {
  const user = await requireAuth();
  const {
    getWorkspaceForUser,
    createWorkspaceTask,
  ...
  const task = createWorkspaceTask(workspaceId, user.id, input);
  addWorkspaceActivity(workspaceId, user.id, 'workspace.task.created', { title: input.title });
  return task;
}

export async function updateWorkspaceTaskStatusRecord(workspaceId, taskId, status) {
  const user = await requireAuth();
  const {
    getWorkspaceForUser,
  ...
  const task = updateWorkspaceTaskStatus(taskId, workspaceId, status);
  if (!task) {
    throw new Error('Task not found.');
  }

  addWorkspaceActivity(workspaceId, user.id, 'workspace.task.status.updated', {
    taskId,
    status,
  });
...
export async function getWorkspaceInviteDetail(token) {
  await requireAuth();
  const { getWorkspaceInvitePreview } = await import('../db/workspaces.js');
  return getWorkspaceInvitePreview(token);
}

export async function acceptWorkspaceInviteRecord(token) {
  const user = await requireAuth();
  const {
  ...
  const preview = getWorkspaceInvitePreview(token);
  if (!preview) {
    throw new Error('Invite not found.');
  }

  const accepted = acceptWorkspaceInvite(token, user.id);
  if (!accepted) {
    throw new Error('Invite could not be accepted.');
  }

  addWorkspaceActivity(preview.workspaceId, user.id, 'workspace.invite.accepted', {
    token,
    userId: user.id,
  });
...
export async function createWorkspaceConversationRecord(workspaceId, input) {
  const user = await requireAuth();
  const {
    getWorkspaceForUser,
  ...
  const conversation = createWorkspaceConversation(workspaceId, user.id, input);
  addWorkspaceActivity(workspaceId, user.id, 'workspace.conversation.created', { title: input.title });
  return conversation;
}

export async function getWorkspaceConversationDetail(workspaceId, conversationId) {
  const user = await requireAuth();
  const {
    getWorkspaceForUser,
    getWorkspaceConversationForUser,
    listWorkspaceMessages,
  } = await import('../db/workspaces.js');
  const workspace = getWorkspaceForUser(workspaceId, user.id);
  if (!workspace) return null;

  const conversation = getWorkspaceConversationForUser(workspaceId, conversationId, user.id);
  if (!conversation) return null;

  const messages = listWorkspaceMessages(conversationId);
  return { workspace, conversation, messages };
}

export async function sendWorkspaceConversationMessage(workspaceId, conversationId, content) {
  const user = await requireAuth();
  const {
    getWorkspaceForUser,
    getWorkspaceConversationForUser,
  ...
  const message = addWorkspaceMessage(conversationId, user.id, {
    role: 'user',
    content,
  });
  addWorkspaceActivity(workspaceId, user.id, 'workspace.message.sent', { conversationId });
  return message;
}
...
export async function getRunnersConfig() {
  ...
  const crons = await readJsonConfigFile(cronsFile);
  const triggers = await readJsonConfigFile(triggersFile);
  return { crons, triggers };
}

export async function getCronsConfig() {
  await requireAuth();
  const { cronsFile } = await import('../paths.js');
  return await readJsonConfigFile(cronsFile);
}

export async function saveCronsConfig(crons) {
  await requireAuth();
  const cron = await import('node-cron');
  const { cronsFile } = await import('../paths.js');

  if (!Array.isArray(crons)) {
    throw new Error('Crons payload must be an array.');
  }

  const normalized = crons.map((entry, index) => {
    const name = String(entry?.name || '').trim();
    const schedule = String(entry?.schedule || '').trim();
    const type = String(entry?.type || 'agent').trim();
    const enabled = entry?.enabled !== false;
    const llmProvider = String(entry?.llm_provider || '').trim();
    const llmModel = String(entry?.llm_model || '').trim();

    if (!name) throw new Error(`Cron ${index + 1} is missing a name.`);
    if (!schedule || !cron.validate(schedule)) {
      throw new Error(`Cron "${name}" has an invalid schedule.`);
    }
    if (!['agent', 'command', 'webhook'].includes(type)) {
      throw new Error(`Cron "${name}" has an unsupported type.`);
    }

    const next = { name, schedule, type, enabled };

    if (type === 'agent') {
      const job = String(entry?.job || '').trim();
      if (!job) throw new Error(`Cron "${name}" needs a job prompt.`);
      next.job = job;
      if (llmProvider) next.llm_provider = llmProvider;
      if (llmModel) next.llm_model = llmModel;
    }

    if (type === 'command') {
      const command = String(entry?.command || '').trim();
      if (!command) throw new Error(`Cron "${name}" needs a command.`);
      next.command = command;
    }

    if (type === 'webhook') {
      const url = String(entry?.url || '').trim();
      if (!url) throw new Error(`Cron "${name}" needs a webhook URL.`);
      next.url = url;
      const method = String(entry?.method || 'POST').trim().toUpperCase();
      next.method = method || 'POST';
      if (entry?.vars && typeof entry.vars === 'object') {
        next.vars = entry.vars;
      }
    }
  ...
  await writeJsonConfigFile(cronsFile, normalized);
  return normalized;
}

export async function getTriggersConfig() {
  await requireAuth();
  const { triggersFile } = await import('../paths.js');
  return await readJsonConfigFile(triggersFile);
}

export async function saveTriggersConfig(triggers) {
  await requireAuth();
  const { triggersFile } = await import('../paths.js');

  if (!Array.isArray(triggers)) {
    throw new Error('Triggers payload must be an array.');
  }

  const normalized = triggers.map((entry, index) => {
    const name = String(entry?.name || '').trim();
    const watchPath = String(entry?.watch_path || '').trim();
    const enabled = entry?.enabled !== false;
    const actions = Array.isArray(entry?.actions) ? entry.actions : [];

    if (!name) throw new Error(`Trigger ${index + 1} is missing a name.`);
    if (!watchPath || !watchPath.startsWith('/')) {
      throw new Error(`Trigger "${name}" needs a watch path starting with "/".`);
    }
    if (!actions.length) {
      throw new Error(`Trigger "${name}" needs at least one action.`);
    }

    const normalizedActions = actions.map((action, actionIndex) => {
      const type = String(action?.type || 'agent').trim();
      if (!['agent', 'command', 'webhook'].includes(type)) {
        throw new Error(`Trigger "${name}" action ${actionIndex + 1} has an unsupported type.`);
      }

      const next = { type };

      if (type === 'agent') {
        const job = String(action?.job || '').trim();
        if (!job) throw new Error(`Trigger "${name}" action ${actionIndex + 1} needs a job prompt.`);
        next.job = job;
        const llmProvider = String(action?.llm_provider || '').trim();
        const llmModel = String(action?.llm_model || '').trim();
        if (llmProvider) next.llm_provider = llmProvider;
        if (llmModel) next.llm_model = llmModel;
      }
      ...
      if (type === 'command') {
        ...
        if (!command) throw new Error(`Trigger "${name}" action ${actionIndex + 1} needs a command.`);
        next.command = command;
      }

      if (type === 'webhook') {
        const url = String(action?.url || '').trim();
        if (!url) throw new Error(`Trigger "${name}" action ${actionIndex + 1} needs a webhook URL.`);
        next.url = url;
        const method = String(action?.method || 'POST').trim().toUpperCase();
        next.method = method || 'POST';
        if (action?.vars && typeof action.vars === 'object') {
          next.vars = action.vars;
        }
      }
  ...
  await writeJsonConfigFile(triggersFile, normalized);
  return normalized;
}
