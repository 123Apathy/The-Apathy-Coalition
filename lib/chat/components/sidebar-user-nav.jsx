...
export function SidebarUserNav({ user, collapsed, version }) {
        ...
        <DropdownMenu>
              ...
              <div className={cn('flex items-center overflow-hidden', collapsed ? '' : 'gap-2')}>
                ...
                {!collapsed && (
                  <div className="min-w-0">
                    {version ? (
                      <div className="truncate text-[10px] uppercase tracking-[0.16em] text-muted-foreground/80">
                        v{version}
                      </div>
                    ) : null}
                    <span className="block truncate text-sm">{user?.email || 'User'}</span>
                  </div>
                )}
              </div>
              {!collapsed && (
          ...
          <DropdownMenuContent align="start" side="top" className="w-56">
            <DropdownMenuItem asChild>
              <Link href="/settings" prefetch className="flex items-center gap-2" style={{ textDecoration: 'inherit', color: 'inherit' }}>
