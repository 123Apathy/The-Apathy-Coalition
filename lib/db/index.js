...
import { ensureUserDataLayout } from '../runtime/user-data.js';
import * as schema from './schema.js';

let _db = null;

function resolveDataDir() {
  return dataDir || path.join(process.env.POPEBOT_REPO_ROOT || process.cwd(), 'data');
}

function resolveDatabasePath() {
  return process.env.DATABASE_PATH || thepopebotDb || path.join(resolveDataDir(), 'thepopebot.sqlite');
}
...
export function getDb() {
  if (!_db) {
    ensureUserDataLayout(PROJECT_ROOT);
