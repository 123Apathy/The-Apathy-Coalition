...
export function acceptWorkspaceInvite(token, userId) {
  ...
  if (existingMembership) {
    db.update(workspaceInvites)
      .set({ status: 'accepted', acceptedByUserId: userId, updatedAt: now() })
...
export function listWorkspaceMembers(workspaceId) {
  const db = getDb();
  const members = db.select().from(workspaceMembers).where(eq(workspaceMembers.workspaceId, workspaceId)).all();
  ...
  return members.map((member) => {
    const account = accountsById.get(member.userId);
    return {
      ...member,
      email: account?.email || null,
      displayName: account?.email || member.userId,
    };
  });
}
...
export function updateWorkspaceTaskStatus(taskId, workspaceId, status) {
  const db = getDb();
  const timestamp = now();
  db.update(workspaceTasks)
    .set({ status, updatedAt: timestamp })
    .where(and(eq(workspaceTasks.id, taskId), eq(workspaceTasks.workspaceId, workspaceId)))
    .run();

  return db.select().from(workspaceTasks)
    .where(and(eq(workspaceTasks.id, taskId), eq(workspaceTasks.workspaceId, workspaceId)))
    .get();
}
