...
function buildRoutingContext(message, attachments = [], options = {}) {
  const attachmentTypes = attachments.map((att) => {
    if (att?.category === 'image') {
      return att?.mimeType === 'application/pdf' ? 'pdf' : 'image';
    }
  ...
  return {
    ...
    codeWorkspace: Boolean(options.repo && options.branch),
...
function isSimpleConversationalMessage(message = '', attachments = [], options = {}) {
  if (attachments?.length > 0) return false;
  if (options.repo || options.branch || options.workspaceId) return false;
  ...
  return words.length <= 4 && /^[a-z0-9 ,.!?'’-]+$/.test(normalized);
}

async function invokeSimpleChat(message, options = {}) {
  const userSettings = options.userId ? buildUserSettingsBundle(options.userId) : null;
  ...
  const systemPrompt = [
    'You are The Apathy Coalition, a helpful local AI assistant.',
...
async function chat(threadId, message, attachments = [], options = {}) {
  if (isSimpleConversationalMessage(message, attachments, options)) {
    persistMessage(threadId, 'user', message || '[attachment]', options);
    const response = await invokeSimpleChat(message, options);
  ...
  const routingContext = buildRoutingContext(message, attachments, options);
  const agent = await getJobAgent(routingContext);
...
async function* chatStream(threadId, message, attachments = [], options = {}) {
  ...
  if (options.repo && options.branch) {
    ...
    agent = await getJobAgent(routingContext);
  }
