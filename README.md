...
**The repository IS the agent** — Every meaningful action in The Apathy Coalition becomes a branch, a commit, a verification step, a PR, a memory entry, or an observable event. You can inspect the whole system instead of trusting a black box.

**Local-first, governed, and inspectable** — The Apathy Coalition combines chat, planning, repository intelligence, memory, change sets, verification, and Control Tower visibility into a single autonomous engineering workspace.

**Self-improving without going feral** — Autonomous changes still flow through branches, approvals, verification, and runtime boundaries. The system can evolve, but it stays reviewable and reversible.
...
In the standard Docker Compose deployment, container control now flows through an internal Job Runner service. The Event Handler no longer needs direct Docker socket access; it talks to the Job Runner over the internal network, and the Job Runner enforces container policy and runtime limits before launching work.

---

## Repository

- GitHub: [123Apathy/The-Apathy-Coalition](https://github.com/123Apathy/The-Apathy-Coalition)
...
| `docker-compose.yml` | Defines how your containers run together (Traefik, event handler, job runner, runner) |
...
thepopebot includes API key authentication, webhook secret validation (fail-closed), session encryption, secret filtering in the Docker agent, auto-merge path restrictions, and a split Job Runner architecture so the Event Handler no longer needs direct Docker socket access in the standard Compose deployment. However, all software carries risk — thepopebot is provided as-is, and you are responsible for securing your own infrastructure. If you're running locally with a tunnel (ngrok, Cloudflare Tunnel, port forwarding), be aware that your dev server endpoints are publicly accessible with no rate limiting and no TLS on the local hop.
...
| [Team Onboarding](docs/TEAM_ONBOARDING.md) | What The Apathy Coalition is, what it can do, and how the full autonomous engineering stack works |
| [Apathy Coalition Platform Spec](docs/APATHY_COALITION_PLATFORM_SPEC.md) | Product architecture for desktop distribution, safe updates, adaptive model installs, and shared workspaces |
| [Apathy Coalition Workspaces V1](docs/APATHY_COALITION_WORKSPACES_V1.md) | First shippable shared workspace model: members, shared chats, tasks, approvals, and workspace memory |
| [Desktop Installer and Updater](docs/APATHY_COALITION_DESKTOP_INSTALLER_UPDATER.md) | Technical design for one-click install, model onboarding, safe updates, and rollback |
| [Apathy Coalition Roadmap](docs/APATHY_COALITION_ROADMAP.md) | Milestones and priorities for turning the current system into a desktop and collaboration product |
| [Auto-Merge](docs/AUTO_MERGE.md) | Auto-merge controls, ALLOWED_PATHS configuration |
| [Deployment](docs/DEPLOYMENT.md) | VPS setup, Docker Compose, HTTPS with Let's Encrypt |
| [WSL VM Setup](docs/WSL_VM_SETUP.md) | Recommended Windows install path using WSL2 as the VM boundary plus a desktop launcher |
